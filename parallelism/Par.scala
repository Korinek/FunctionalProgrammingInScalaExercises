...
object Par {
   ...
   * `fork(map2(a,b)(f))` if we want the evaluation of `f` to occur in a
   * separate thread.*/
  ...
  def parFilter[A](l: List[A])(p: A => Boolean): Par[List[A]] = {
    val pars: List[Par[List[A]]] = l map ((asyncF((a: A) => if (p(a)) List(a) else List())))
...
object Examples {
  ...
  /* `IndexedSeq` is a superclass of random-access sequences like `Vector` in the
   * standard library. Unlike lists, these sequences provide an efficient `splitAt`
   * method for dividing them into two parts at a particular index.*/
